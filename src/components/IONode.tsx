...
export default function IONode({ node, value, ports, onBodyDown, onBodyUp, onPort, onPortUp, pending }: IONodeProps) {
      ...
      {isIn ? (
        ...
        <>
          <rect x={node.x} y={node.y} width={w} height={h} rx={9}
            ...
            onPointerDown={(e) => onBodyDown(e, node)}
            onPointerUp={(e) => onBodyUp(e, node)} style={{ cursor: "grab" }} />
          ...
          <g
            ...
            {pending && pending.node === node.id && (
              <circle className="gw-armed-ring" cx={ports.inputs[0].x} cy={ports.inputs[0].y} r={9}
                fill="none" stroke={C.accent} strokeWidth={2} style={{ pointerEvents: "none" }} />
            )}
